...
const { setParameter, setCalibration } = useRouter();
...
async function updateParam(p: ModuleParam, newVal: string | number) {
    ...
    if (m) {
        ...
        if (pp) {
            pp.value = newVal;
            pp.pendingValue = String(newVal);
            pp.pendingUpdate = Date.now();
        }
    }
    await setParameter(state.selected.r, state.selected.c, p.id, p.dt, String(newVal));
}
...
// Optimistically pin a value in the store without sending to device.
// Used while the color picker is open so periodic polls don't reset the UI.
function previewParamValue(p: ModuleParam, newVal: string) {
    if (!state.selected) return;
    const key = `${state.selected.r},${state.selected.c}`;
    const m = state.modules[key];
    if (m) {
...
async function startCalibration(p: ModuleParam) {
    ...
    await setCalibration(state.selected.r, state.selected.c, p.id, extremeMin, extremeMax);
...
async function applyCalibration(p: ModuleParam) {
    ...
    await setCalibration(state.selected.r, state.selected.c, p.id, Math.floor(p.calibMin), Math.floor(p.calibMax));
                    ...
                    <div v-else-if="p.dt === 3" class="led-controls" @click.stop>
                        <input type="color" class="param-input-color"
                            ...
                            @input="(e) => {
                                ...
                                previewParamValue(p, `${r},${g},${b},${parseLEDValue(p.value).status}`);
                            }"
                            @change="(e) => {
                                const hex = (e.target as HTMLInputElement).value;
                                const r = parseInt(hex.substr(1, 2), 16);
