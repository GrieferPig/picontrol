...
class PortPacked(ctypes.Structure):
    ...
    _fields_ = [
        ...
        ("txPin", ctypes.c_uint8),
        ("rxPin", ctypes.c_uint8),
    ]
...
def cmd_modules_list(pcs: PicontrolSerial, args: argparse.Namespace) -> int:
    ...
    for i in range(port_count):
        ...
        print(f"  TX/RX Pins: {port.txPin}/{port.rxPin}")
...
def cmd_map_set_curve(pcs: PicontrolSerial, args: argparse.Namespace) -> int:
    ...
    # Parse curve data (expecting 16 bytes as hex string or comma-separated values)
    if args.curve.startswith("0x") or all(
        c in "0123456789abcdefABCDEF" for c in args.curve
    ):
        # Hex string
        curve_hex = args.curve.replace("0x", "").replace(" ", "")
        if len(curve_hex) != 32:  # 16 bytes = 32 hex chars
            print(
                f"Error: Curve data must be 16 bytes (32 hex chars), got {len(curve_hex)//2} bytes"
            )
            return 1
        ...
        try:
            ...
            if len(values) != 16:
                print(f"Error: Curve data must be 16 bytes, got {len(values)}")
                return 1
